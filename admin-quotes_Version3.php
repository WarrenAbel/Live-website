...
// NOTE: SELECT * already includes payfast_* fields if they exist in the table
...
<head>
  ...
  <style>
    ...
    .pf-box { margin-top:0.5rem; padding-top:0.5rem; border-top:1px dashed #d1d5db; }
    .pf-label { font-weight:600; color:#111827; font-size:0.75rem; }
    .pf-row { font-size:0.75rem; color:#111827; margin-top:0.15rem; }
  </style>
</head>
<body>
  <div class="topbar">
    ...
    <tbody>
      ...
      <?php else: ?>
        <?php foreach ($quotes as $q): ?>
          <?php
            $statusClass = 'status-' . ($q['status'] ?? 'draft');
            $paymentUrl  = 'https://' . $_SERVER['HTTP_HOST'] . '/quote-payment.php?id=' . $q['id'];

            // PayFast fields (will be present only if you added the DB columns)
            $pfStatus  = $q['payfast_payment_status'] ?? '';
            $pfId      = $q['payfast_pf_payment_id'] ?? '';
            $pfGross   = $q['payfast_amount_gross'] ?? '';
            $pfUpdated = $q['payfast_updated_at'] ?? '';
          ?>
          <tr>
            ...
            <td>
              ...
              <?php if ($pfStatus || $pfId || $pfGross || $pfUpdated): ?>
                <div class="pf-box">
                  <div class="pf-label">PayFast</div>
                  <?php if ($pfStatus): ?><div class="pf-row">Status: <strong><?php echo h($pfStatus); ?></strong></div><?php endif; ?>
                  <?php if ($pfId): ?><div class="pf-row">pf_payment_id: <strong><?php echo h($pfId); ?></strong></div><?php endif; ?>
                  <?php if ($pfGross !== '' && $pfGross !== null): ?><div class="pf-row">amount_gross: <strong>R <?php echo h(fmtMoney($pfGross)); ?></strong></div><?php endif; ?>
                  <?php if ($pfUpdated): ?><div class="pf-row">updated_at: <strong><?php echo h($pfUpdated); ?></strong></div><?php endif; ?>
                </div>
              <?php endif; ?>
            </td>
            <td>
