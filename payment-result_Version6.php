...
/**
 * PayFast status override:
 * If PayFast has already told us CANCELLED/COMPLETE, reflect that in the page
 * even if the main `status` field hasn't been updated yet.
 */
$pfStatusNorm = strtoupper(trim((string)($pfStatus ?? '')));
if ($pfStatusNorm === 'CANCELLED') {
    $status = 'cancelled';
} elseif ($pfStatusNorm === 'COMPLETE') {
    $status = 'paid';
}
